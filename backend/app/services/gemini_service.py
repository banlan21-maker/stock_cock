...
async def extract_stocks_from_keyword(keyword: str) -> list[dict]:
    """키워드로부터 관련 한국 상장 종목을 Gemini 지식 베이스로 추출한다."""
    prompt = f"""당신은 한국 주식시장 전문가입니다.
키워드 "{keyword}"와 관련된 한국거래소(KRX) 상장 종목을 추출해주세요.

[핵심 원칙]
- 종목명과 6자리 종목코드가 100% 확실한 경우에만 포함하세요
- 조금이라도 불확실하면 해당 종목은 제외하세요
- 관련 종목이 없거나 확실한 종목이 없으면 빈 배열([])을 반환하세요
- 억지로 채우지 마세요. 10개를 반드시 채울 필요 없습니다

[포함 기준]
- 해당 키워드 사업을 실제로 영위하는 기업
- 키워드 관련 매출 비중이 의미 있는 기업
- 종목코드와 종목명이 확실히 일치하는 경우만

응답은 반드시 JSON 배열만 출력 (다른 텍스트 없이):
[
  {{"code": "6자리종목코드", "name": "종목명", "reason": "이 키워드와 직접 연관된 이유 한 문장"}},
  ...
]"""
    try:
